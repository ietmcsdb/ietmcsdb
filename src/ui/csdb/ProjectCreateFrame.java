...
public class ProjectCreateFrame extends JDialog {
	...
	private static Logger log=Logger.getLogger(ProjectCreateFrame.class.toString());;
	
	public ProjectCreateFrame(final Connection conn) {

		setBounds(100, 100, 550, 502);
		final JDialog obj=this;
	...
	public void inseritem2protable(Connection conn) throws SQLException {
		...
		/*for(int i=0;i<8;++i)
			if(checkBoxs.get(i).isSelected())
				sum=pow_int(10,i);*/
		String dmtype=String.format("%08d",sum);
		...
		{
			boolean res=proDAO.insertProjectItem(proitem);
			if(res==true) JOptionPane.showConfirmDialog(null,"插入数据成功","信息", JOptionPane.ERROR_MESSAGE);
			else JOptionPane.showConfirmDialog(null,"插入数据失败","信息", JOptionPane.ERROR_MESSAGE);
			this.dispose();
		}
